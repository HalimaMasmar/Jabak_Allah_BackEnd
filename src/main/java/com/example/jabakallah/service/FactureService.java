...
import com.example.jabakallah.repository.FactureRepo;
...
public class FactureService {
    ...
    public boolean PayerRecharge(int value,String phoneNum){
        UserApp user = userRepo.findByNumTel(phoneNum);
        Compte compte = user.getCompte();
        if (value <= compte.getSolde()){
            Double solde = compte.getSolde();
            compte.setSolde(solde-value);
            compteRepo.save(compte);
            return true;
        }
        else {
      ...
      public List<Facture> getListFacturesOfCreancier(String creancier,String ownerphone){
          ...
          for (Facture fct : factureRepo.findAllByStatus("unpaied")){
    ...
    public List<Facture> getListFacturePaid(String ownerphone){
        List<Facture> factures = new ArrayList<>();
        for (Facture fct : factureRepo.findAllByStatus("payed")){
            if (fct.getOwnerphone().equals(ownerphone)){
